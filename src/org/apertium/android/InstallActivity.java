...
public class InstallActivity extends Activity implements OnClickListener {
	...
	private void initView() {
	    ...
	    String name = extras.getString("filename");    		
	    this._packageID =  name.substring(0, name.length() - 4);
	...
	private void run2(){	
	    Thread t = new Thread() {
	        ...
	        public void run() {
				...
				FileManager.move(AppPreference.TEMP_DIR()+"/"+config.PackageID(),AppPreference.BASE_DIR()+"/"+config.PackageID());
